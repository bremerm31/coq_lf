...
Proof.
  ...
  - induction m as [|m' IHm'].
    ...
    + simpl. symmetry. rewrite <- IHm'. simpl. rewrite -> plus_n_Sm. reflexivity.
Qed.
...
Proof.
  ...
  - intros p'. rewrite <- plus_n_Sm. rewrite -> (add_comm (S m') p'). rewrite <- plus_n_Sm.
    rewrite <- plus_n_Sm. rewrite -> ( add_comm (S (n+m')) p'). rewrite <- plus_n_Sm.
    rewrite -> (add_comm p' (n+m')). rewrite -> (add_comm p' m'). rewrite IHm'. reflexivity.
Qed.
(** [] *)
...
Proof.
  induction n.
  - reflexivity.
  - simpl. rewrite <- (negb_involutive (even n)). rewrite <- IHn. simpl. reflexivity.
Qed.
...
Proof.
  ...
  assert ( H0: m + p = p + m ).
  { rewrite add_comm. reflexivity. }
  rewrite H0. rewrite add_assoc. rewrite add_comm. reflexivity.
Qed.
